...
class ProductAttributes extends Component {
  ...
  // receives product and renders it's attributes, used in the shopping cart and on the product page
  render() {
    return (
      <div className={`product-attributes ${this.props.static ? 'static-attributes' : ''}`}>
                    ...
                    (
                      <div key={itemIndex} className={`attribute-swatch-border ${isSelected ? 'selected-attribute-swatch' : ''}`} >
                        <button
                          ...
                          onClick={!isSelected && !this.props.static ? () => { this.props.attributeClickHandler(index, itemIndex) } : null}
                        >
                        </button>
                      </div>
                    )
